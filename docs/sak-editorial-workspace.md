...
The first live Sol run confirmed Sol/high in all seven calls and completed the brief, but the reference validator falsely rejected a source compound split across PDF lines (a hyphen immediately followed by a newline). Evidence normalization now joins only that layout break, retaining the hyphen and all letters. Regression checks retain rejection of inserted spaces, removed hyphens and changed meaning. This formatting fix applies to both brief and article evidence checks.
