...
export const NOTICE_EDITORIAL_PROMPT_VERSION = "v5.12.2";
...
SPRÅK OG SITATER
- Maks åtte ord i title, med ett konkret poeng og selskapsnavn fremfor ticker. Kontroller tittelens betydning separat: korrekt fordeling eller presisering i body reparerer ikke en tittel som gir totalen feil navn eller omfang. Bruk nøkterne verb, korte setninger og korrekt æ, ø og å. Utelat selskapsendelsen ASA. Lead er normalt én eller to setninger. Lead + body må holde tegngrensen i oppgaven; tittel og metadata teller ikke med.
...
- Kontroller sammenhengen etter forkorting: «resten», «tilsvarende mengde», «halvparten» og lignende må vise til riktig tall, enhet og delmengde før uttrykket, eller eksplisitt i samme setning. En løs opplysning etterpå eller et annet tall tidligere er ikke et tydelig grunnlag. Behold nødvendig kildebelagt grunnlag eller utelat hele den relative påstanden; ikke fyll inn et ukjent tall. Siste avsnitt skal tilføre noe, ikke gjenta at avtalen inngås, dialogen fortsetter eller kjøpet avbrytes.
...
const KIND_INSTRUCTIONS: Record<NoticePromptKind, string> = {
  ...
  yearly: "ÅRSRAPPORT, LEDERLØNN: Hold saken til lederlønn og godtgjørelse. Prioriter lederens rolle, samlet godtgjørelse, sammenligning og vesentlig fordeling på fastlønn, bonus og aksjer. Bruk navn bare når det entydig er knyttet til den aktuelle godtgjørelsen; ellers bruk rollen. Behold det faktiske regnskapsåret, valuta/skala og konsern/morselskap. Direkte betaling fra morselskapet er ikke nødvendigvis konsernets samlede godtgjørelse. Bruk rapportert total, ikke summen av avrundede delbeløp, og ikke utled en presis prosentendring fra grovt avrundede tall. Skill regnskapsført/tildelt aksjeverdi fra realisert gevinst og kontantutbetaling. Drift og resultater skal ikke fylle hullet hvis lønnstall mangler; noter i stedet mangelen i source_limitations og sett importance til uviktig."
};
