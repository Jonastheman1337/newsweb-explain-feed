...
export const NOTICE_EDITORIAL_PROMPT_VERSION = "v5.12.3";
...
- Briefens mustInclude angir vesentlige fakta, ikke ferdig formulert tekst eller hele kildegrunnlaget. Oppgavetypens emneavgrensning gjelder også når briefen velger feil tema. Bevar kildebelagte forbehold og avgjørende hendelser innenfor denne avgrensningen selv om briefen utelater dem. Fakta som trengs for å forstå hendelsen må stå i lead/body, ikke bare key_facts. Flett dem sammen uten gjentakelser. En ny redigeringsinstruksjon kan endre vinkel og utvalg innenfor kildegrunnlaget.
...
- Ved oppkjøp og finansiering: skill samlet verdi, kjøperens betaling, utbytte fra målselskapet, utsatt/resultatavhengig betaling, gjeld og aksjevederlag. Navngi betaler/mottaker av vesentlige deler, og hva som må skje før betaling. Når en aksjonærverdi inkluderer målselskapets særutbytte, si uttrykkelig at målselskapet betaler det; budgiverens navn og ordet særutbytte alene er ikke nok. Erklæring av utbytte, gjennomføring og utbetaling er ulike hendelser; utbytte kan erklæres før overtakelse og bare utbetales hvis kjøpet gjennomføres. Behold vesentlige godkjennelser. Ikke gjør maksimal ramme eller mulig opsjonsverdi til sikker inntekt.
...
- Et valgt totalmål må beholde materialet/virksomheten det måler og vesentlig sammensetning. En blanding av materialer er ikke bare én deltype, prosessert innmating er ikke utvunnet produksjon, og antall lanserte modeller er ikke solgte enheter. Behold kildens konkrete fordeling når den endrer forståelsen av totalen; ikke legg til unødvendige delposter. En forklaring på tidspunkt/forsinkelse er ikke en forklaring på mengdens størrelse.
...
SPRÅK OG SITATER
- Maks åtte ord i title, med ett konkret poeng og selskapsnavn fremfor ticker. Kontroller tittelens betydning separat: korrekt presisering i body reparerer ikke feil navn, omfang eller rekordgrense i title. «Høyest siden omstart» dokumenterer ikke en uavgrenset rekord; behold tidsgrensen i tittelen eller velg en nøktern tittel uten rekordpåstand. Bruk korte setninger og korrekt æ, ø og å. Utelat selskapsendelsen ASA. Lead er normalt én eller to setninger. Lead + body må holde tegngrensen i oppgaven; tittel og metadata teller ikke med.
...
- Forklar nødvendige fagord gjennom det de betyr her. «Tap issue» kan gjengis som utvidelse av et eksisterende obligasjonslån; det er en språkforklaring, ikke nye finansieringsvilkår. Behold mulig/dialog/samtykke og ikke legg til beløp, rente, sikkerhet eller avtalt finansiering. EBITDA skal ved første bruk forklares som «resultat før renter, skatt, av- og nedskrivninger (ebitda)». EBIT er driftsresultat. Utelat fagordet/måltallet når forklaringen ikke fortjener plassen.
- Kontroller sammenhengen etter forkorting: «resten», «tilsvarende mengde», «halvparten» og lignende trenger riktig tall, enhet og delmengde før uttrykket eller eksplisitt i samme setning. Kildens relative sammenligning kan løses opp til det entydige foregående kildebeløpet når om lag/mulig og riktig materiale beholdes. Et kildebelegg må da dekke både grunnlaget og koblingen. Er koblingen uklar, ikke sett inn et tall. Er sammenligningen uvesentlig, fjern hele den relative påstanden og behold eventuell selvstendig driftsopplysning. Ikke gjeninnfør et løsrevet sammenligningsuttrykk under retting. Siste avsnitt skal tilføre noe fremfor å gjenta status.
...
const KIND_INSTRUCTIONS: Record<NoticePromptKind, string> = {
  ...
  report: "RESULTATRAPPORT: Velg den materielle utviklingen, en endret prognose eller annen vesentlig hendelse som vinkel. Bevar relevante inntekter og resultatlinje med samme periode i fjor når de finnes, samt kildefast forklaring eller vesentlig utsikt. En avgjørende nedleggelse/endring i driften eller en separat vesentlig aksjonærutbetaling som rapporten melder, må ikke forsvinne bak resultatvinkelen eller ordinært utbytte. Behold driftsomfang og vilkår for en ekstra utbetaling; ikke gjør gammel bakgrunn til en ny hendelse. Bruk resultatoppstillingens måltall og kolonneoverskrifter. Ikke la rapportpublisering erstatte innholdet. Ikke gjør helår til kvartal eller beregn manglende kvartal ved subtraksjon.",
  yearly: "ÅRSRAPPORT, LEDERLØNN: Hold bestilling og artikkel til lederlønn og godtgjørelse. Prioriter lederens rolle, rapportert total, sammenligning og vesentlig fordeling på fastlønn, bonus og aksjer. Bruk navn bare når det entydig er knyttet til godtgjørelsen; ellers bruk rollen. Behold faktisk regnskapsår, valuta/skala og konsern/morselskap. Enhet kan støttes av en uttrykkelig notelenke fra samme konsern-/selskapsoppstilling med samsvarende note, perioder og beløp; lån ikke enheten fra en urelatert nabotabell. Morselskapets direkte betaling er ikke nødvendigvis konsernets godtgjørelse. Uttrykkelig avgrenset ikke-betaling er kildeinnhold, ikke bevis for at hele konsernet betaler null. Bruk rapportert total fremfor summen av avrundede delbeløp; ikke utled presis prosentendring fra grov avrunding. Skill regnskapsført/tildelt aksjeverdi fra realisert gevinst og kontantutbetaling. Rapportpublisering, aksjonærutbytte, drift og resultater kan ikke erstatte manglende lønnsgrunnlag, heller ikke ved tvungen generering. Beskriv mangelen i sourceLimitations/source_limitations; manglende data er ikke null lønn. Uten publiserbar godtgjørelsesopplysning er importance uviktig."
};

/** The planner, writer and coverage reviewer must use the same topic boundary. */
export function createNoticeKindInstructions(kind: NoticePromptKind): string {
...
- En tidligere oppgitt likviditetsfrist eller et finansieringsbehov kan forklare dagens uavklarte prosess. Behold kildens fulle dato med år når anslagstidspunktet er nødvendig, og perioden/fristen anslaget gjaldt. Ikke gjør gammelt anslag til ny bekreftelse eller slutt at pengene er brukt opp. Datoen kan dekke følgende tydelig historiske setninger i samme body-avsnitt bare når de viser til den samme ene prior-kilden. Nytt avsnitt/felt, nåtidsopplysning, annen kilde eller tidsmarkør avslutter denne koblingen; ved blanding/tvil gjentas full dato i den berørte setningen. Sibling/korrigering trenger egen tydelig relasjon og status i setningen.
