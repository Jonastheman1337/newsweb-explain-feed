...
for (const [index, source] of cases.entries()) {
    ...
    try {
        const result = await generateFastDraft(payload, model, started + 35000, (request) => callOpenAIForJson(client, request), ({ content, ...telemetry }) => calls.push({ ...telemetry, output: JSON.parse(content) }));
...
const report = { testedAt: new Date().toISOString(), model, syntheticSources: true, results };
