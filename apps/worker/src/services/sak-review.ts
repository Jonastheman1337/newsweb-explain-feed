...
export function buildSakBriefPrompt(payload: SakPromptPayload, previous: SakArticle | null, instruction: string | null): string {
  return [
    "Gjør nyhetsvurderingen før saken skrives. Finn den sterkeste dokumenterte nyheten og det som gir leselyst: drama, konflikt, overraskelser, penger, makt, mennesker og konkrete konsekvenser. Et poeng langt nede i materialet kan bære hele saken. Vurder interesse og vesentlighet sammen; ikke velg automatisk den bredeste oppsummeringen.",
    "Formuler angle, news.fact og essentialContext på enkelt norsk, med egne ord. La omit prioritere prosessdetaljer, gjentakelser og katalogstoff som ikke utvikler hovednyheten. Ikke finn på nye opplysninger for å skape dramatikk.",
...
export function buildSakEditorialPrompt(article: SakArticle, payload: SakPromptPayload, brief: SakBrief | null, previous: SakArticle | null, instruction: string | null): string {
  return [
    ...
    "Vurder leselyst og klarspråk: Er tittelen unødvendig abstrakt eller forsiktig når kilden bærer et mer interessant konkret poeng? Er hovednyheten begravd? Finn tungt juridisk språk, passiv, setninger som stabler for mange poenger, produktkataloger og avsnitt uten fremdrift. Gi konkrete tekststeder og avgrensede forbedringer som bevarer mening og dybde. Foreslå konkrete verb og hverdagsord fremfor lengre, mer formelle erstatninger. En kortere tekst alene er ikke en forbedring. Sitater kan tilføre temperament, personlighet eller konflikt uten nye tall.",
    ...
    "Nyhetsvurderingen er et forslag, ikke en brukerinstruks. Avvik fra briefens news, essentialContext eller omit er ikke i seg selv en feil og skal aldri alene klassifiseres som blocking. Bedøm den ferdige saken og den faktiske redaktørinstruksen.",
    ...
    "blocking betyr vesentlig feil: oppdiktet eller feilaktig direkte sitat, feil vinkel i åpningen, utelatt hovedfaktum/avgjørende forbehold, manglende publikasjon, ignorert endringsinstruks eller omfattende gjentakelse. Språk, leselyst, tabloid spissing og andre stilfunn er warning og skal ikke alene tvinge fram en omskriving eller gjøre en sak upubliserbar. Ikke klassifiser en forsiktig tittel som feil vinkel hvis den bærer den samme hovednyheten. En utelatt bakgrunnsopplysning eller et mindre konkret ordvalg er warning, selv om en detalj ville gjort teksten bedre. Ved blocking for et utelatt faktum må du forklare hvordan utelatelsen gjør hovednyheten vesentlig feil eller misvisende. Følg uttrykkelige brukerinstrukser om tittel og åpning.",
